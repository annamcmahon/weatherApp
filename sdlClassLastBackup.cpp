...
void sdlClass::CreateTextTextures() {
	// temp += "\xB0";
	varStr =temp.at(5)+ ", "+ temp.at(6);
	
	
	//string varStr = "\xB0"; // degree symbol
	varStr =temp.at(1) + "\xB0";
	...
	int lastBottom = textRect.h - textRect.y;
        ...
        string highAndLow = "high:" + temp.at(1)+ "  |    "+ "low:" + temp.at(1);
	string weatherCondition = temp.at(1);
	
	moreWeatherDetails.push_back(highAndLow);
	moreWeatherDetails.push_back(weatherCondition);
	
	surf = TTF_RenderText_Solid( font, varStr.c_str(), textColor );
        textTexture = SurfaceToTexture( surf );
        SDL_QueryTexture( textTexture, NULL, NULL, &w, &h );
	 textRect.w = textRect.w/4;  textRect.h = textRect.w/5;
        textRect.y = SCREENHEIGHT/2 + 100; textRect.x = SCREENWIDTH/2+ (textRect.x/2);

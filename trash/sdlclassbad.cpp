...
SDL_Texture* sdlClass::CreateImageTextures(string filenameStr,int imageDestRectx, int imageDestRecty, int imageDestRectw, int imageDestRecth) {
	SDL_Texture* newImageTexture;
     newImageTexture = load_texture(("imgs/"+filenameStr+".bmp").c_str(), renderer); //CHANGED "img_test.png"
     // We need to create a destination rectangle for the image (where we want this to be show) on the renderer area
     imageDestRect.x = 0; imageDestRecty = 0;
     imageDestRectw = 1776; imageDestRecth = 952;
}

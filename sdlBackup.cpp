...
void sdlClass::CreateTextTextures() {
	...
	textRect.x = SCREENWIDTH/2 - (textRect.w)/2;
        textRect.y = SCREENHEIGHT/3 - (textRect.h)/2;
	int bottomOfLast = textRect.y + textRect.h;
	 textures.push_back(textTexture);
        ...
        varStr = "high:" + temp.at(3)+"\xB0"+"    |     "+" low:" + temp.at(4)+"\xB0";
	 surf = TTF_RenderText_Solid( font, varStr.c_str(), textColor );
        textTexture = SurfaceToTexture( surf );
        SDL_QueryTexture( textTexture, NULL, NULL, &textRect.w , &textRect.h );
	 textRect.w = textRect.w/4;
        textRect.h = textRect.h/4;
        textRect.y = bottomOfLast +20;
	textRect.x = SCREENWIDTH/2 - (textRect.x);
